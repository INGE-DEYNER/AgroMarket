...
import com.agromarket.application.api.request.EmailVerificationRequest;
import com.agromarket.application.api.request.LoginRequest;
import com.agromarket.application.api.request.PasswordResetConfirmRequest;
import com.agromarket.application.api.request.PasswordResetRequest;
import com.agromarket.application.api.request.RegistroRequest;
import com.agromarket.application.api.request.TwoFactorCodeRequest;
import com.agromarket.application.api.request.TwoFactorLoginRequest;
import com.agromarket.application.api.request.VerificarCorreoRequest;
import com.agromarket.application.api.request.VerifyCodeRequest;
import com.agromarket.application.api.response.ApiResponse;
import com.agromarket.application.api.response.AuthResponse;
import com.agromarket.application.api.response.TwoFactorSetupResponse;
...
public class AuthController {
    ...
    private final com.agromarket.domain.ports.PasswordResetService passwordResetService;
    private final com.agromarket.domain.ports.EmailVerificationService emailVerificationService;
    ...
    public ResponseEntity<ApiResponse<java.util.Map<String, Boolean>>> verificarCorreo(@Valid @RequestBody VerificarCorreoRequest request) {
        com.agromarket.application.persistence.sql.entities.UsuarioEntity usuario = emailVerificationService.verifyCode(request.getCorreo(), request.getCodigo());
    ...
    public ResponseEntity<ApiResponse<java.util.Map<String, Boolean>>> verificar(@Valid @RequestBody EmailVerificationRequest request) {
        com.agromarket.application.persistence.sql.entities.UsuarioEntity usuario = emailVerificationService.verifyToken(request.getToken());
        boolean pendiente = usuario.getRol() != null && usuario.getRol().name().equals("PRODUCTOR");
        return ResponseEntity.ok(ApiResponse.<java.util.Map<String, Boolean>>builder()
                .success(true)
                .message(pendiente ? "Correo verificado. Tu cuenta está pendiente de aprobación por un administrador." : "Correo verificado")
