...
public class CuponController {
    ...
    private final com.agromarket.application.persistence.sql.repositories.CuponDescuentoRepository cuponRepository;

    @GetMapping
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<List<com.agromarket.application.persistence.sql.entities.CuponDescuento>>> listarCupones(
            @AuthenticationPrincipal JwtUserPrincipal principal) {
        List<com.agromarket.application.persistence.sql.entities.CuponDescuento> cupones = cuponService.obtenerCuponesUsuario(principal.getUserId());
        return ResponseEntity.ok(ApiResponse.<List<com.agromarket.application.persistence.sql.entities.CuponDescuento>>builder()
                .success(true)
    ...
    public ResponseEntity<ApiResponse<List<com.agromarket.application.persistence.sql.entities.CuponDescuento>>> listarTodos() {
        return ResponseEntity.ok(ApiResponse.<List<com.agromarket.application.persistence.sql.entities.CuponDescuento>>builder()
                .success(true)
    ...
    @PostMapping
    @PreAuthorize("hasRole('ADMINISTRADOR')")
    public ResponseEntity<ApiResponse<com.agromarket.application.persistence.sql.entities.CuponDescuento>> crear(
            @RequestBody com.agromarket.application.persistence.sql.entities.CuponDescuento cupon) {
        if (cupon.getFechaExpiracion() == null) {
            cupon.setFechaExpiracion(java.time.LocalDateTime.now().plusDays(30));
        }
        return ResponseEntity.ok(ApiResponse.<com.agromarket.application.persistence.sql.entities.CuponDescuento>builder()
                .success(true)
