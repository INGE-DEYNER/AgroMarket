...
export default function CompletarCuentaModal({ onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!cedula.trim()) {
      setError(t('completar.error.documento', 'El número de documento es obligatorio'));
      return;
    }

    if (!fechaNacimiento) {
      setError(t('completar.error.fecha', 'La fecha de nacimiento es obligatoria'));
      return;
    }
    ...
    if (age < 18) {
      setError(t('completar.error.edad', 'Debes ser mayor de 18 años para usar la plataforma'));
      return;
    }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {t('completar.subtitulo', 'Por regulaciones legales, debes completar los siguientes datos antes de continuar.')}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label style={{ fontSize: '13px', fontWeight: '600', color: '#4a5d4e' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <input
              type="text"
              placeholder={t('completar.placeholder.numDoc', 'Ingresa tu número de documento')}
              value={cedula}
          ...
          >
            {loading ? t('completar.guardando', 'Guardando...') : t('completar.guardar', 'Guardar y Continuar')}
          </button>
        </form>
      </div>
    </div>
