  ...
  return (
    <div className="catalog-card" style={{ cursor: 'pointer' }}>
      ...
      <div className="catalog-card-body">
        {p.tipoFruta && <div className="catalog-card-tipo" onClick={() => onViewDetails?.(p)}>{p.tipoFruta}</div>}
        <div className="catalog-card-name" onClick={() => onViewDetails?.(p)} style={{ fontWeight: '700', fontSize: '1.05rem', margin: '4px 0 8px 0' }}>{p.nombre}</div>
        <div className="catalog-card-producer" style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '8px' }}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
          </svg>
          <span 
            onClick={(e) => { e.stopPropagation(); onContactProducer?.(p); }} 
            style={{ textDecoration: 'underline', color: 'var(--primary)', fontWeight: '600', cursor: 'pointer' }}
          >
        ...
        <div className="catalog-card-rating" onClick={() => onViewDetails?.(p)}>
          ★★★★★
          <span>({p.calificacion || '4.8'})</span>
        </div>
        <div className="catalog-card-footer">
          <div className="catalog-card-price" onClick={() => onViewDetails?.(p)}>
