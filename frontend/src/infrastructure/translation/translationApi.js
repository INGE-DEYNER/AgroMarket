...
export async function translateText(text, source = 'auto', target = 'en') {
  ...
  for (let i = 0; i < FALLBACK_INSTANCES.length; i++) {
    const instance = FALLBACK_INSTANCES[(currentInstanceIndex + i) % FALLBACK_INSTANCES.length];
    ...
    try {
      ...
      if (response.ok) {
        ...
        currentInstanceIndex = (currentInstanceIndex + i) % FALLBACK_INSTANCES.length;
