...
export default function ReportarProblema() {
  ...
  return <PublicLayout><div className="page-wrap report-page">
    ...
    <section className="report-card">
      ...
      {sent ? <div className="report-success"><span className="report-success-badge">✓</span><h2>{t("report.successTitle")}</h2><p>{t("report.successText")}</p><button type="button" onClick={() => { setSent(false); setForm({ category: "general", subject: "", description: "", email: "" }); }}>{t("report.newReport")}</button></div> : <form className="report-form" onSubmit={submit}>
