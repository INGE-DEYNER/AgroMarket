...
export default function ReportarProblema() {
  ...
  return <PublicLayout><div className="page-wrap report-page">
    <nav className="breadcrumb"><span>{t("nav.home")}</span><span>›</span><strong>{t("report.title")}</strong></nav>
    <section className="report-card">
      <div className="report-heading"><span className="report-icon">!</span><div><h1>{t("report.title")}</h1><p>{t("report.subtitle")}</p></div></div>
      {sent ? <div className="report-success"><h2>{t("report.successTitle")}</h2><p>{t("report.successText")}</p><button type="button" onClick={() => { setSent(false); setForm({ category: "general", subject: "", description: "", email: "" }); }}>{t("report.newReport")}</button></div> : <form className="report-form" onSubmit={submit}>
    ...
    <section className="report-help"><h2>{t("report.helpTitle")}</h2><p>{t("report.helpText")}</p><button type="button" onClick={() => document.querySelector(".chatbot-trigger")?.click()}>{t("report.openAssistant")}</button></section>
  </div></PublicLayout>;
}
