...
export default function Catalogo() {
  ...
  const toggleWishlist = async (product) => {
    try {
      await api.post(`/lista-deseos/${product.id}`);
    } catch (error) {
  ...
  const visible = useMemo(() => {
    const productNameOf = (p) =>
      p.nombre ?? p.name ?? t("catalog.productFallback", "Producto");
    const productCategoryOf = (p) =>
      p.categoria ?? p.tipo ?? p.category ?? p.fruitType ?? "";
    ...
    let list = products.filter((p) => {
      const text =
        `${productNameOf(p)} ${producerName(p)} ${productCategoryOf(p)}`.toLowerCase();
      const matchesQuery =
      ...
      const matchesCategory =
        ...
        (!isMaracuyaValue(selectedCategory)
          ? false
          : productCategoryOf(p).toLowerCase() ===
    ...
    switch (sort) {
      ...
      case "nombreAsc":
        list = [...list].sort((a, b) =>
          productNameOf(a).localeCompare(productNameOf(b), locale),
        );
        break;
      default:
        break;
      ...
      <div className="catalog-page">
        <div className="catalog-container">
          ...
          <div className="catalog-layout-grid">
            {/* ─── SIDEBAR DE FILTROS ─── */}
            <aside
              className={`catalog-filters-sidebar${filtersOpen ? " open" : ""}`}
            >
              <button
                type="button"
              ...
              <div className="catalog-filter-group">
                ...
                <div style={{ fontSize: "0.85rem", fontWeight: 600 }}>
                  {t("catalog.upTo", "Hasta:")} {formatearPrecio(priceMax)} ·{" "}
                  {divisaActual}
                </div>
              </div>
            </aside>
                ...
                <div className="catalog-grid">
                  {visible.map((p) => {
                    const nombre =
                      p.nombre ??
                      p.name ??
                      t("catalog.productFallback", "Producto");
                    const categoria =
                      p.categoria ?? p.tipo ?? p.category ?? p.fruitType ?? "";
                    const prodName =
                      p.productorNombre ??
