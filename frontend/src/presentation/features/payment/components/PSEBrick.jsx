      ...
      try {
        ...
        if (typeof globalThis.MercadoPago !== 'undefined') {
          ...
          const pseBanks = paymentMethods.filter(
            method => method.payment_type_id === 'bank_transfer' && method.id.includes('pse')
          );
        ...
        setBanks([
          { id: 'pse', name: 'PSE - Pagos Seguros en Línea', financialInstitution: 'Todos los bancos' },
          { id: 'pse_bancolombia', name: 'Bancolombia', financialInstitution: 'Bancolombia' },
          { id: 'pse_davivienda', name: 'Davivienda', financialInstitution: 'Davivienda' },
          { id: 'pse_banco_de_bogota', name: 'Banco de Bogotá', financialInstitution: 'Banco de Bogotá' },
          { id: 'pse_banco_occidente', name: 'Banco Occidente', financialInstitution: 'Banco Occidente' },
          { id: 'pse_banco_popular', name: 'Banco Popular', financialInstitution: 'Banco Popular' },
        ]);
      }
    };
  ...
  return (
    <div className="w-full space-y-4" data-payment-theme="adaptive">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-300">
        Selecciona tu banco para realizar el pago mediante transferencia electrónica:
      </p>
        ...
        {banks.map((bank) => (
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="font-medium text-gray-800 dark:text-gray-200">{bank.name}</p>
                {bank.financialInstitution && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">{bank.financialInstitution}</p>
                )}
              </div>
            </div>
          </button>
      ...
      {selectedBank && (
        <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          <p className="text-blue-700 dark:text-blue-300">
            Banco seleccionado: <span className="font-semibold">{selectedBank.name}</span>
          </p>
          <p className="text-sm text-blue-600 dark:text-blue-400 mt-1">
            Serás redirigido a la página de pago de {selectedBank.financialInstitution || selectedBank.name}
          </p>
        </div>
      )}
